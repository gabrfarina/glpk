...
static int
basis_col(void *info, int j, int ind[],
          mpq_t val[]) { /* this auxiliary routine provides row indices and
                            numeric values of non-zero elements in j-th column
                            of the matrix B */
  SSX *ssx = info;
  int m = ssx->m;
  int n = ssx->n;
  ...
  if (k <= m) { /* it is a column of the unity matrix I */
    len = 1, ind[1] = k, mpq_set_si(val[1], 1, 1);
  } else { /* it is a column of the original constraint matrix -A */
    len = 0;
...
void ssx_eval_bbar(SSX *ssx) {
  ...
  for (j = 1; j <= n; j++) {
    ...
    if (mpq_sgn(x) == 0)
      continue;
    k = Q_col[m + j]; /* x[k] = xN[j] */
    if (k <= m) {     /* N[j] is a column of the unity matrix I */
      mpq_sub(bbar[k], bbar[k], x);
    } else { /* N[j] is a column of the original constraint matrix -A */
      for (ptr = A_ptr[k - m]; ptr < A_ptr[k - m + 1]; ptr++) {
...
void ssx_eval_row(SSX *ssx) {
  ...
  mpq_t temp;
  mpq_init(temp);
  for (j = 1; j <= n; j++) { /* ap[j] := - N'[j] * rho (inner product) */
    k = Q_col[m + j];        /* x[k] = xN[j] */
    if (k <= m)
      mpq_neg(ap[j], rho[k]);
    else {
...
void ssx_eval_col(SSX *ssx) {
  ...
  if (k <= m) {     /* N[q] is a column of the unity matrix I */
    mpq_set_si(aq[k], 1, 1);
  } else { /* N[q] is a column of the original constraint matrix -A */
    for (ptr = A_ptr[k - m]; ptr < A_ptr[k - m + 1]; ptr++)
...
void ssx_chuzc(SSX *ssx) {
  ...
  for (j = 1; j <= n; j++) {
    ...
    if ((stat[k] == SSX_NF || stat[k] == SSX_NL) && s < 0 ||
        (stat[k] == SSX_NF || stat[k] == SSX_NU) &&
            s > 0) { /* reduced cost of xN[j] indicates possible improving of
                        the objective function */
      mpq_abs(abs_cur, cbar[j]);
      if (q == 0 || mpq_cmp(abs_cur, abs_best) > 0) {
        q = j;
        q_dir = -s;
        mpq_set(abs_best, abs_cur);
      }
    }
  }
...
void ssx_chuzr(SSX *ssx) {
  ...
  for (i = 1; i <= m; i++) {
    ...
    if (s < 0) {    /* xB[i] decreases */
      k = Q_col[i]; /* x[k] = xB[i] */
      t = type[k];
      if (t == SSX_LO || t == SSX_DB ||
          t == SSX_FX) { /* xB[i] has finite lower bound */
    ...
    } else if (s > 0) { /* xB[i] increases */
      k = Q_col[i];     /* x[k] = xB[i] */
      t = type[k];
      if (t == SSX_UP || t == SSX_DB ||
          t == SSX_FX) { /* xB[i] has finite upper bound */
...
void ssx_update_bbar(SSX *ssx) {
  ...
  mpq_init(temp);
  xassert(1 <= q && q <= n);
  if (p < 0) { /* xN[q] is double-bounded and goes to its opposite bound */
    /* nop */;
  } else { /* xN[q] becomes xB[p] in the adjacent basis */
    /* xB.new[p] = xN[q] + delta xN[q] */
    xassert(1 <= p && p <= m);
...
void ssx_delete(SSX *ssx) {
  ...
  for (i = 0; i <= m; i++)
    mpq_clear(ssx->bbar[i]);
  xfree(ssx->bbar);
  for (i = 1; i <= m; i++)
    mpq_clear(ssx->pi[i]);
  xfree(ssx->pi);
  for (j = 1; j <= n; j++)
    mpq_clear(ssx->cbar[j]);
  xfree(ssx->cbar);
  for (i = 1; i <= m; i++)
    mpq_clear(ssx->rho[i]);
  xfree(ssx->rho);
